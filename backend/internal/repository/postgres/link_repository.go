...
func (r *LinkRepository) Create(ctx context.Context, link *model.Link) error {
	...
	err := r.db.QueryRowContext(ctx, query, link.Code, link.OriginalURL, link.CreatedAt, link.ExpiresAt).Scan(&link.ID)
	return err
}
...
func (r *LinkRepository) IncrementClicks(ctx context.Context, code string) error {
	query := `UPDATE links SET clicks = clicks + 1, last_clicked_at = $1 WHERE code = $2`
